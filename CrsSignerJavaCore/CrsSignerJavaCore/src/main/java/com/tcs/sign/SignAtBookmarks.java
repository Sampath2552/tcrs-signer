...
//        ByteArrayInputStream bais = new ByteArrayInputStream(barr);
        ...
        int noOfPages = doc.getNumberOfPages();
        int lastPage = noOfPages - 1;
        int bookMarkPage = currentBookmark.getPageNumber();
        ...
        signatureOptions.setVisualSignature(createVisualSignatureTemplate(doc, bookMarkPage, rect, signature));
        signatureOptions.setPage(bookMarkPage);
...
//        ByteArrayInputStream bais = new ByteArrayInputStream(barr);
        ...
        int noOfPages = doc.getNumberOfPages();
        int lastPage = noOfPages - 1;
        int bookMarkPage = currentBookmark.getPageNumber() == 0 ? lastPage : currentBookmark.getPageNumber(); ;
    ...
    public static void main(String[] args) throws UnrecoverableKeyException, CertificateException, KeyStoreException, IOException, NoSuchAlgorithmException, OperatorCreationException, CMSException {
        ...
        String contextPath = "C:\\Users\\ACER\\Desktop\\CRS_Files\\";
        //String contextPath = "C:\\Users\\v1017860\\Desktop\\SignedFiles\\";
        String[] paths = {"99996_31032025_RA02", "99996_31032025_RW01",
                "99996_31032025_RW35CL",
                "00005_31032025_RW35NWR",
                "00005_31032025_RW38",
                "00001_CCDP_31122024_AUDIT_SUMMARY"};
        File[] files = new File(contextPath).listFiles();

        for (File f : files) {
            //String currentPath= contextPath+s+".pdf";
            // File f = new File(currentPath);
            if (f.isFile()) {
                System.out.println(f.getName());
                String name = f.getName();
                byte[] fileBarr = Files.readAllBytes(f.toPath());
                //System.out.println(Base64.getEncoder().encodeToString(fileBarr));
                byte[] signedContent = (byte[]) SignAtBookmarks.configureSignatureForByteArrayWithWindowsMy("Password123", fileBarr, "checker", "ANIL BABU KAYALORATH").get("signedContent");

                File file = new File("C:\\Users\\ACER\\Desktop\\SignedFiles\\" + "checker"+name);
