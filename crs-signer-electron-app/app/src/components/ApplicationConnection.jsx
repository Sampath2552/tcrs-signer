...
const ApplicationConnection = ({crsLoggedInUserId})=>{
    ...
    return (<>
        ...
        {(crsLoggedInUserId!=='' && (crsLoggedInUserId!==userId)) && <Alert severity="error">
            You have logged in as {userId} in CRS Digital Signer and as {crsLoggedInUserId} in CRS . Please make sure that you have logged in with the same ID in both application
        </Alert>}
        <Typography sx={{fontSize:"16px",lineHeight:"24px",color:"rgba(0,0,0,0.6)",fontWeight:400, whiteSpace:"pre-wrap"}}>
            In case no application is listed below, go to connection section in the application and choose connect CRS Digital Signer.
        </Typography>
